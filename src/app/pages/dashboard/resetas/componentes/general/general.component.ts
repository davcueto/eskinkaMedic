...
import { convertImageToBase64 } from '../../../../../utils/image-util';
...
export class GeneralComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.buildPDF();
    // this.previewPDF();
    this.formSub = this.formCer.valueChanges.pipe(debounceTime(500)).subscribe(async (value) => {
      // await this.buildPDF();
      this.previewPDF();
    });
  }
  ...
  async buildPDF() {
    ...
    this.doc.text('DR. JOSE GABRIEL CASAHONDA ESQUINCA.', 105, 30, { align: 'center' });
    this.doc.text('Céd. Prof. 701460 SSA 89437', 105, 36, { align: 'center' });
    this.doc.text('Médico General Certificado', 105, 42, { align: 'center' });
    this.doc.text('U. A. E. M.', 105, 49, { align: 'center' });
    this.doc.line(20, 54, 190, 54);
    this.doc.setLineWidth(0.5);
    this.doc.line(20, 55, 190, 55);

    try {
      const base64 = await this.convertImageToBase64('assets/simbolo.png');
      this.doc.addImage(base64, 'PNG', 15, 25, 30, 20);
      this.doc.addImage(base64, 'PNG', 165, 25, 30, 20);
    } catch (error) {
      console.error('Error cargando la imagen', error);
    }

    this.doc.setFont('helvetica', 'bold');
    this.doc.text('DR. JOSE GABRIEL CASAHONDA ESQUINCA', 105, 250, { align: 'center' });
    this.doc.setLineWidth(0.5);
  ...
  async setImage() {
    convertImageToBase64('assets/snake.jpg')
      .then((base64) => {
        console.log("obtenido", base64)
        this.doc.addImage(base64, 'JPEG', 15, 40, 180, 100);
      })
      .catch((error) => {
